...
describe('authentification', () => {
  it('connecte un utilisateur valide et masque le mot de passe', async () => {
    const res = await api.mockLogin('admin', 'admin123');
  ...
  it('rejette un mot de passe invalide', async () => {
    await expect(api.mockLogin('admin', 'faux')).rejects.toThrow('INVALID_CREDENTIALS');
  });

  it('connecte un encadreur via son endpoint dédié', async () => {
    const res = await api.mockEncadreurLogin('encadreur1', 'encadreur123');
    expect(res.user.role).toBe('ENCADREUR');
    expect(res.user.encadreurId).toBe('ENC-001');
  });

  it("refuse la connexion encadreur à un rôle non-encadreur", async () => {
    await expect(api.mockEncadreurLogin('admin', 'admin123')).rejects.toThrow('INVALID_CREDENTIALS');
  });
});

describe('bordereaux', () => {
  it('liste les bordereaux avec champs dérivés', async () => {
    const items = await api.mockGetBordereaux();
  ...
  it('filtre par région et par type', async () => {
    ...
    const filtered = await api.mockGetBordereaux({ region });
    expect(filtered.every((b) => b.region === region)).toBe(true);
  });

  it('détecte les doublons', async () => {
    expect(await api.mockCheckDuplicate(PILGRIM.idNumber, 2027)).toBe(true);
    expect(await api.mockCheckDuplicate('0000', 2027)).toBe(false);
  });
  ...
  it('crée un bordereau et refuse un doublon', async () => {
    ...
    const created = await api.mockCreateBordereau(payload, ADMIN);
...
describe('reporting', () => {
  it('agrège les indicateurs clés', async () => {
    const rep = await api.mockGetReporting();
...
describe('inscription en ligne et par encadreur', () => {
  ...
  it('inscrit un pèlerin en ligne et génère un mot de passe', async () => {
    const res = await api.mockRegisterPilgrimOnline(base);
    expect(res.id).toMatch(/^BOR-/);
  });

  it('refuse un doublon en ligne', async () => {
    await api.mockRegisterPilgrimOnline({ ...base, idNumber: '5550002222' });
    await expect(api.mockRegisterPilgrimOnline({ ...base, idNumber: '5550002222' })).rejects.toThrow();
  });

  it('inscrit un pèlerin via un encadreur avec identifiants', async () => {
    const res = await api.mockRegisterPilgrimByEncadreur(
  ...
  it('importe une liste de pèlerins pour un encadreur', async () => {
    ...
    const res = await api.mockImportPilgrims(rows, 'ENC-001', { username: 'encadreur1' });
...
describe('versements', () => {
  ...
  it('rejette un versement au dossier introuvable ou montant invalide', async () => {
    const d = await payableDossier();
    await expect(
      api.mockCreateVersementOnline('0000', '0000', { method: 'AGENCE', amount: 1000, reference: 'x' })
    ).rejects.toThrow();
    await expect(
      api.mockCreateVersementOnline(d.idNumber, d.phone, { method: 'AGENCE', amount: -5, reference: 'x' })
    ).rejects.toThrow();
  });
  ...
  it('valide puis liste les versements en attente et l’historique', async () => {
    ...
    const res = await api.mockValidateVersement(target.bordereauId, target.id, ADMIN);
    expect(res.amountPaid).toBeGreaterThan(0);
    const history = await api.mockGetVersementsHistory({ status: 'VALIDE' });
  ...
  it('rejette un versement en attente', async () => {
    ...
    const res = await api.mockRejectVersement(target.bordereauId, target.id, 'référence introuvable', ADMIN);
...
describe('paiements groupés', () => {
  it('recherche un bénéficiaire par CNI', async () => {
    const found = await api.mockLookupBeneficiary(PILGRIM.idNumber, 2027);
    expect(found.found).toBe(true);
    expect(found.name).toBeTruthy();
    const missing = await api.mockLookupBeneficiary('0000', 2027);
  ...
  it('crée un versement groupé et le liste', async () => {
    ...
    const res = await api.mockCreateGroupedVersementOnline(PILGRIM.idNumber, PILGRIM.phone, {
    ...
    const groups = await api.mockGetGroupedPayments();
...
describe('visa', () => {
  it('change le statut visa d’un bordereau', async () => {
    const all = await api.mockGetBordereaux();
    const res = await api.mockChangeVisaStatus(all[0].id, 'EN_COURS', 'test', ADMIN);
    expect(res.visaStatus).toBe('EN_COURS');
  });

  it('change le statut en masse pour un encadreur', async () => {
    const all = await api.mockGetBordereaux({ encadreurId: 'ENC-001' });
    const res = await api.mockBulkChangeVisaStatus(
  ...
  it('importe des statuts visa et détecte les anomalies', async () => {
    const all = await api.mockGetBordereaux();
    const res = await api.mockImportVisaStatuses(
      [{ idNumber: all[0].idNumber, status: 'ACCORDE' }], ADMIN
    );
    expect(res).toBeTruthy();
    const anomalies = await api.mockCheckStatusAnomalies();
...
describe('remboursements', () => {
  it('marque les versements à rembourser après un visa refusé puis les traite', async () => {
    ...
    await api.mockChangeVisaStatus(withPayment.id, 'REFUSE', 'refus', ADMIN);
    const refunds = await api.mockGetRefunds();
    expect(Array.isArray(refunds)).toBe(true);
    if (refunds.length > 0) {
      const r = refunds[0];
      const res = await api.mockProcessRefund(r.bordereauId, r.versementId, {
...
describe('référentiels (encadreurs, saisons, utilisateurs)', () => {
  it('CRUD encadreur', async () => {
    const created = await api.mockCreateEncadreur({ name: 'Nouveau Guide', region: 'Centre' }, ADMIN);
    expect(created.id).toMatch(/^ENC-/);
    const updated = await api.mockUpdateEncadreur(created.id, { active: false }, ADMIN);
    expect(updated.active).toBe(false);
    const list = await api.mockGetEncadreurs({ onlyActive: false });
    expect(list.some((e) => e.id === created.id)).toBe(true);
  });

  it('importe des encadreurs', async () => {
    const res = await api.mockImportEncadreurs([{ name: 'Import Guide', region: 'Nord' }], ADMIN);
    expect(res.created.length).toBeGreaterThan(0);
  });

  it('CRUD saison + prix officiel', async () => {
    const created = await api.mockCreateSeason(
    ...
    const price = await api.mockGetOfficialPrice(2030, 'PELERIN', true);
    expect(price).toBe(3150000);
    const updated = await api.mockUpdateSeason(2030, { isOpen: false }, ADMIN);
    expect(updated).toBeTruthy();
  });

  it('CRUD utilisateur et refus de doublon', async () => {
    const created = await api.mockCreateUser(
    ...
    await expect(api.mockCreateUser({ username: 'nouvel_op', password: 'x', role: 'OPERATEUR_HADJ', name: 'X' }, ADMIN)).rejects.toThrow();
    const updated = await api.mockUpdateUser(created.id, { active: false }, ADMIN);
    expect(updated.active).toBe(false);
  });

  it('importe des utilisateurs', async () => {
    const res = await api.mockImportUsers(
...
describe('SMTP et réinitialisation du mot de passe', () => {
  it('lit et met à jour les paramètres SMTP', async () => {
    const settings = await api.mockGetSmtpSettings();
    expect(settings).toBeTruthy();
    const updated = await api.mockUpdateSmtpSettings({ ...settings, fromName: 'Test Hadj' }, ADMIN);
    expect(updated.fromName).toBe('Test Hadj');
  });

  it('répond identiquement que le compte existe ou non (anti-énumération)', async () => {
    const known = await api.mockRequestPasswordReset('admin');
    const unknown = await api.mockRequestPasswordReset('inconnu@x.cm');
    expect(known.sent).toBe(true);
    expect(unknown.sent).toBe(true);
  });

  it('rejette un OTP invalide', async () => {
    await api.mockRequestPasswordReset('admin');
    await expect(api.mockResetPasswordWithOtp('admin', '000000', 'nouveaupass')).rejects.toThrow('INVALID_OTP');
  });
  ...
  it('réinitialise le mot de passe avec le bon OTP', async () => {
    ...
    if (otp) {
      const res = await api.mockResetPasswordWithOtp('admin', otp, 'nouveaupass');
      expect(res.success).toBe(true);
      const login = await api.mockLogin('admin', 'nouveaupass');
...
describe('commissions, pèlerin, passeports, audit', () => {
  it('calcule les commissions encadreurs', async () => {
    const rows = await api.mockGetEncadreurCommissions(2027);
  ...
  it('connecte un pèlerin par CNI + téléphone et charge son groupe', async () => {
    const res = await api.mockPilgrimLogin(PILGRIM.idNumber, PILGRIM.phone);
    expect(res.idNumber).toBe(PILGRIM.idNumber);
    const group = await api.mockGetEncadreurGroup('ENC-001');
    expect(Array.isArray(group)).toBe(true);
  });

  it('refuse une connexion pèlerin invalide', async () => {
    await expect(api.mockPilgrimLogin('0000', '0000')).rejects.toThrow();
  });

  it('gère les dépôts de passeports', async () => {
    const deposits = await api.mockGetPassportDeposits(2027);
    expect(deposits).toBeTruthy();
    const all = await api.mockGetBordereaux();
    const res = await api.mockTogglePassportDeposit(all[0].id, true, ADMIN);
    expect(res).toBeTruthy();
  });

  it('expose le journal d’audit', async () => {
    await api.mockChangeVisaStatus((await api.mockGetBordereaux())[0].id, 'EN_COURS', 'x', ADMIN);
    const logs = await api.mockGetAuditLogs();
    expect(logs.length).toBeGreaterThan(0);
  });

  it('liste les saisons', async () => {
    const seasons = await api.mockGetSeasons();
